set_property PACKAGE_PIN W5 [get_ports clk]
    set_property IOSTANDARD LVCMOS33 [get_ports clk]
    set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets start_IBUF]

    create_clock -add -name sys_clk_pin -period 10.00 -waveform {0 5} [get_ports clk]						

set_property IOSTANDARD LVCMOS33 [get_ports {d[11]}]
set_property PACKAGE_PIN V17 [get_ports {d[11]}]	
				
set_property IOSTANDARD LVCMOS33 [get_ports {d[10]}]
set_property PACKAGE_PIN V16 [get_ports {d[10]}]					

set_property IOSTANDARD LVCMOS33 [get_ports {d[9]}]
set_property PACKAGE_PIN W16 [get_ports {d[9]}]

set_property IOSTANDARD LVCMOS33 [get_ports {d[8]}]
set_property PACKAGE_PIN W17 [get_ports {d[8]}]

set_property IOSTANDARD LVCMOS33 [get_ports {d[7]}]
set_property PACKAGE_PIN W15 [get_ports {d[7]}]

set_property IOSTANDARD LVCMOS33 [get_ports {d[6]}]
set_property PACKAGE_PIN V15 [get_ports {d[6]}]

set_property IOSTANDARD LVCMOS33 [get_ports {d[5]}]
set_property PACKAGE_PIN W14 [get_ports {d[5]}]

set_property IOSTANDARD LVCMOS33 [get_ports {d[4]}]
set_property PACKAGE_PIN W13 [get_ports {d[4]}]

set_property IOSTANDARD LVCMOS33 [get_ports {d[3]}]
set_property PACKAGE_PIN V2 [get_ports {d[3]}]

set_property IOSTANDARD LVCMOS33 [get_ports {d[2]}]
set_property PACKAGE_PIN T3 [get_ports {d[2]}]

set_property IOSTANDARD LVCMOS33 [get_ports {d[1]}]
set_property PACKAGE_PIN T2 [get_ports {d[1]}]

set_property IOSTANDARD LVCMOS33 [get_ports {d[0]}]
set_property PACKAGE_PIN R3 [get_ports {d[0]}]
	
	
set_property IOSTANDARD LVCMOS33 [get_ports {led[11]}]
set_property PACKAGE_PIN U16 [get_ports {led[11]}]	
				
set_property IOSTANDARD LVCMOS33 [get_ports {led[10]}]
set_property PACKAGE_PIN E19 [get_ports {led[10]}]					

set_property IOSTANDARD LVCMOS33 [get_ports {led[9]}]
set_property PACKAGE_PIN U19 [get_ports {led[9]}]

set_property IOSTANDARD LVCMOS33 [get_ports {led[8]}]
set_property PACKAGE_PIN V19 [get_ports {led[8]}]

set_property IOSTANDARD LVCMOS33 [get_ports {led[7]}]
set_property PACKAGE_PIN W18 [get_ports {led[7]}]

set_property IOSTANDARD LVCMOS33 [get_ports {led[6]}]
set_property PACKAGE_PIN U15 [get_ports {led[6]}]

set_property IOSTANDARD LVCMOS33 [get_ports {led[5]}]
set_property PACKAGE_PIN U14 [get_ports {led[5]}]

set_property IOSTANDARD LVCMOS33 [get_ports {led[4]}]
set_property PACKAGE_PIN V14 [get_ports {led[4]}]

set_property IOSTANDARD LVCMOS33 [get_ports {led[3]}]
set_property PACKAGE_PIN V13 [get_ports {led[3]}]

set_property IOSTANDARD LVCMOS33 [get_ports {led[2]}]
set_property PACKAGE_PIN V3 [get_ports {led[2]}]

set_property IOSTANDARD LVCMOS33 [get_ports {led[1]}]
set_property PACKAGE_PIN W3 [get_ports {led[1]}]

set_property IOSTANDARD LVCMOS33 [get_ports {led[0]}]
set_property PACKAGE_PIN U3 [get_ports {led[0]}]

set_property IOSTANDARD LVCMOS33 [get_ports {enable}]
set_property PACKAGE_PIN T17 [get_ports {enable}]

set_property IOSTANDARD LVCMOS33 [get_ports {start}]
set_property PACKAGE_PIN W19 [get_ports {start}]

set_property IOSTANDARD LVCMOS33 [get_ports {reset}]
set_property PACKAGE_PIN U17 [get_ports {reset}]



set_property IOSTANDARD LVCMOS33 [get_ports {error}]
set_property PACKAGE_PIN P1 [get_ports {error}]

set_property IOSTANDARD LVCMOS33 [get_ports {clk_out}]
set_property PACKAGE_PIN L1 [get_ports {clk_out}]


set_property IOSTANDARD LVCMOS33 [get_ports {segout[6]}]
set_property PACKAGE_PIN W7 [get_ports {segout[6]}]

set_property IOSTANDARD LVCMOS33 [get_ports {segout[5]}]
set_property PACKAGE_PIN W6 [get_ports {segout[5]}]

set_property IOSTANDARD LVCMOS33 [get_ports {segout[4]}]
set_property PACKAGE_PIN U8 [get_ports {segout[4]}]

set_property IOSTANDARD LVCMOS33 [get_ports {segout[3]}]
set_property PACKAGE_PIN V8 [get_ports {segout[3]}]

set_property IOSTANDARD LVCMOS33 [get_ports {segout[2]}]
set_property PACKAGE_PIN U5 [get_ports {segout[2]}]

set_property IOSTANDARD LVCMOS33 [get_ports {segout[1]}]
set_property PACKAGE_PIN V5 [get_ports {segout[1]}]

set_property IOSTANDARD LVCMOS33 [get_ports {segout[0]}]
set_property PACKAGE_PIN U7 [get_ports {segout[0]}]




set_property IOSTANDARD LVCMOS33 [get_ports {an[0]}]
set_property PACKAGE_PIN U2 [get_ports {an[0]}]

set_property IOSTANDARD LVCMOS33 [get_ports {an[1]}]
set_property PACKAGE_PIN U4 [get_ports {an[1]}]

set_property IOSTANDARD LVCMOS33 [get_ports {an[2]}]
set_property PACKAGE_PIN V4 [get_ports {an[2]}]

set_property IOSTANDARD LVCMOS33 [get_ports {an[3]}]
set_property PACKAGE_PIN W4 [get_ports {an[3]}]
















	